    ...
    return(
        <div className="flex items-center bg-zinc-900 rounded-xl h-16 px-6 gap-3 shadow-shape">
            ...
            {props.isGuestsInputOpen ? (
                <button className="bg-zinc-800 text-zinc-200 flex items-center px-5 py-2 gap-2 rounded-lg font-medium hover:bg-zinc-700 w-56" onClick={props.closeGuestInput}>
                  Alterar local/data
                  <Settings2 className="size-5"/>
                </button>
              ):(
                <button className="bg-lime-300 text-lime-950 flex items-center px-5 py-2 gap-2 rounded-lg font-medium hover:bg-lime-400" onClick={props.openGuestsInput}>
                  Continuar
